...
#                    os.rmdir(parent_dir)
        ...
        def output_frames(self, num_div=1) :
                ...
                while (time < end) :
                    ...
                    if (numpy.amax(cell) > 0) :
                        ...
                        print numpy.amax(cell)
                        camera = self.detector_output(cell)
                        #camera.astype('uint%d' % (self.configs.detector_ADC_bit))
                        #camera = camera*(40000./27000.)

                        # save image to file
                        #toimage(camera).save(self.image_file_name)

                        #from matplotlib.backends.backend_pdf import PdfPages

                        print camera
                        print camera[camera.shape[0] / 2, camera.shape[1] / 2]
                        print numpy.amax(camera)
                        pylab.imshow(camera)
                        #pylab.colorbar()
                        pylab.savefig('test_model.png')
        ...
        def detector_output(self, cell, bc=None) :
                ...
                #PE = numpy.zeros(Nw_pixel * Nh_pixel)
